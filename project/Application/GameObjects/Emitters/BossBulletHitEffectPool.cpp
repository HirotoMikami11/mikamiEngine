...
BossBulletHitEffectPool::BossBulletHitEffectPool()
...
void BossBulletHitEffectPool::Initialize(DirectXCommon* dxCommon, const std::string& presetName, const std::string& emitterName)
{
	dxCommon_ = dxCommon;
...
void BossBulletHitEffectPool::CreateParticleInstances()
{
	if (!particleEditor_) {
		return;
	}
...
int BossBulletHitEffectPool::GetNextAvailableIndex()
{
	// ラウンドロビン方式で空きスロットを検索
	for (size_t i = 0; i < kPoolSize_; ++i) {
...
bool BossBulletHitEffectPool::TriggerEffect(const Vector3& position, const Vector3& direction)
{
	// 次に使用可能なインデックスを取得
...
void BossBulletHitEffectPool::ActivateEffect(size_t index, const Vector3& position, const Vector3& direction)
{
...
bool BossBulletHitEffectPool::IsEffectFinished(size_t index) const
{
	if (index >= kPoolSize_ || !effects_[index].instance) {
		return true;
	}
...
void BossBulletHitEffectPool::Update()
{
...
size_t BossBulletHitEffectPool::GetActiveEffectCount() const
{
	size_t count = 0;
	for (size_t i = 0; i < kPoolSize_; ++i) {
		if (effects_[i].isActive) {
...
void BossBulletHitEffectPool::Finalize()
{
	if (!particleEditor_) {
		return;
	}
...
void BossBulletHitEffectPool::ImGui()
{
#ifdef USEIMGUI
	if (ImGui::TreeNode("Boss Bullet Hit Effect Pool")) {
